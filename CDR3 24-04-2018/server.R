    ...
    observeEvent(is.null(input$begin),{
      ...
      output$optlev <- renderTable({
        ...
        if(input$opt_type == "Identity"){
          ...
          #fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[1],Clus$seqnum[t1[1]+1],OptNew(AminoCl(t1[1],lastlist,df,flagtic,logFile),TRUE,logFile)) )
          #for(i in 2:length(t1)){
          #  if(i<= length(xm2)){
              #fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[i],Clus$seqnum[t1[i]+1],OptNew(AminoCl(t1[i],lastlist,df,flagtic,logFile),FALSE,logFile)),sep = "\n")
          ...
          #  }else{
          #    #fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d        :    %s',t1[i],Clus$seqnum[t1[i]+1],OptNew(AminoCl(t1[i],lastlist,df,flagtic,logFile),FALSE,logFile)),sep = "\n")
