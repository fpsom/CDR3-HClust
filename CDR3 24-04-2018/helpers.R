...
LogoLev <- function(lev,lastlist,Clus,df,flagtic,logFile){
  if(flagtic == TRUE) tic()
  if( is.element(lev,lastlist$clep) == FALSE){
    ...
    for(i in 1:length(t1)){
      if(i<= length(xm2)){
        ...
        names(listff)[length(listff)] = sprintf('Cluster.%d - seqnum:%d - leaf', t1[i],Clus$seqnum[t1[i]+1]) 
      }else{
        ...
        if (nrow(x1) >0){
          ...
          listff$temp = x1
          names(listff)[length(listff)] = sprintf('Cluster.%d - seqnum:%d', t1[i],Clus$seqnum[t1[i]+1]) 
        } 
      }
    }
    if(flagtic == TRUE){
...
OptNew <- function(df,logFile){
  if(is.list(df)){
    ...
    #max3 = max(max1,max2)
    #if(poso == TRUE){
    #  paste(str_pad(xarpos,max3,side = "right", pad = "~"),collapse = ' ')
    #}else{
    #  paste(str_pad(xar,max3,side = "right", pad = "~"),collapse = ' ')
    #}
    #datatable(rbind(xar,xarpos))
    #paste(xar,collapse = ' ')
    rbind(xar,xarpos)
  }
}
