...
idenlev <- function(lev,Clus,cho,flagtic,logFile){
  ...
  if(cho == "Identity"){
    ...
    sum(na.omit(Clus[Clus$level == lev,]$Identity)) /  length(na.omit(Clus[Clus$level == lev,]$Identity))
    ...
    sum(na.omit(Clus[Clus$level == lev,]$Similarity)) /  length(na.omit(Clus[Clus$level == lev,]$Similarity))
  }
}
...
EmPin <- function(lastlist,Clus,dfsd,flagtic,logFile){
  ...
  for (i in 0:(max(na.omit(lastlist$clep))) ) {
    ...
    t4 = sd(na.omit(Clus[Clus$level == i,]$Similarity))
