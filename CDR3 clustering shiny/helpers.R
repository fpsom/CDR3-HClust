...
NetwInt <- function(lev,thr,thrt,netyp,df,lastlist,Clus){
  df = df[ do.call( order , df[ , match(  colnames(df[str_which(names(df), "level.")]) , names(df) ) ]  ) , ]
  df_args <- c(df[str_which(names(df), "level.")], sep="/")
  if(lev == 19){
    df$pathString<- do.call(paste, df_args)
  ...
  net0links = as_edgelist(graph = net0)
  net0links = as.data.frame(net0links)
  net1links = as_edgelist(graph = net1)
  ...
  n0 = network(net0links,  vertex.attr=net0nodes, matrix.type="edgelist",loops=F, multiple=F, ignore.eval = F)
  ...
  if(netyp == "whole"){
    ...
    render.d3movie(nn0, usearrows = F, displaylabels = TRUE, bg="#ffffff", 
                   ...
                   vertex.cex = nn0 %v% "size", label = nn0 %v% "label", edge.lwd = nn0 %e% "width" , edge.col = nn0 %e% "color",output.mode = "htmlWidget",script.type= 'embedded')
  }else{
    render.d3movie(nn1, usearrows = F, displaylabels = TRUE, bg="#ffffff", 
                   vertex.border="#ffffff", vertex.col =  nn1 %v% "color",
                   vertex.cex = nn1 %v% "size", label = nn1 %v% "label", edge.lwd = nn1 %e% "width" , edge.col = nn1 %e% "color",output.mode = "htmlWidget", script.type= 'embedded')
    #nn1
  }
}
