...
Choice <- function(list3,let,sim,d){
  ...
  cel = which(permat[,3:ncol(permat)] == max(permat[,3:ncol(permat)]), arr.ind = TRUE)
  ela = 1
  poss = max(permat[,3:ncol(permat)]) 
  
  if (max(permat[,3:ncol(permat)]) == 100 ){ # We exclude the 100 % from the max values
    cel = which(permat[,3:ncol(permat)] == max(permat[,3:ncol(permat)][permat[,3:ncol(permat)] !=max(permat[,3:ncol(permat)])]), arr.ind = TRUE) # The desired cell
    poss = max(permat[,3:ncol(permat)][permat[,3:ncol(permat)] !=max(permat[,3:ncol(permat)])])
  }
  if ((length(cel)/2) > 1){
    eqcol = vector('numeric') # A vector for the cell's rows with the same entropy
    j = 1
    for(i in 1:(length(cel)/2)){
      if (permat[,3:ncol(permat)][nrow(permat[,3:ncol(permat)]),cel[i,2]] < permat[,3:ncol(permat)][nrow(permat[,3:ncol(permat)]),cel[ela,2]]) {  # If the percentage is the same (cel multidimensional) we keep the cel with the lowest entropy
        ela = i
        eqcol[] = 0
        eqcol[1] = i
        j = 2
      }else if(permat[,3:ncol(permat)][nrow(permat[,3:ncol(permat)]),cel[i,2]] == permat[,3:ncol(permat)][nrow(permat[,3:ncol(permat)]),cel[ela,2]]){
        eqcol[j] = i
        j = j+1
      }
    }
    ...
    if (length(eqcol)>1 && nn == FALSE){ # If the vector has 2 or more numbers means that we have columns with the same entropy and nn = FALSE in order not to double check the persim
      ela = eqcol[1]
      meg = persim[,3:ncol(permat)][str_which(sim,let[cel[ela,1]]),cel[ela,2]]
      for(i in 2:length(eqcol)){
        if(persim[,3:ncol(permat)][str_which(sim,let[cel[eqcol[i],1]]),cel[eqcol[i],2]] > meg){ # We find the maximum percentage of the similarity percentage marix
          meg = persim[,3:ncol(permat)][str_which(sim,let[cel[eqcol[i],1]]),cel[eqcol[i],2]]
          ela = eqcol[i]
        } else if (persim[,3:ncol(permat)][str_which(sim,let[cel[eqcol[i],1]]),cel[eqcol[i],2]] == meg){ # If the percentage is the same we use the entropy
          if(persim[,3:ncol(permat)][nrow(persim[,3:ncol(permat)]),cel[eqcol[i],2]] < persim[,3:ncol(permat)][nrow(persim[,3:ncol(permat)]),cel[ela,2]]){
            ela = i
          }
        }
      }
...
Divide <- function(list4,let,sim,d){
  ...
  x1 = str_which(str_detect(str_sub(udata[udata$clusters == br,]$AA.JUNCTION,(cel[ela,2]+2),(cel[ela,2]+2)), let[cel[ela,1]]), "TRUE")
  y1 = udata[udata$clusters == br,]$AA.JUNCTION
  z1 = y1[x1]
  x2 = str_which(str_detect(str_sub(udata[udata$clusters == br,]$AA.JUNCTION,(cel[ela,2]+2),(cel[ela,2]+2)), let[cel[ela,1]]), "FALSE")
  ...
  udata[udata$clusters == br,]$clusters <- ifelse(str_detect(str_sub(udata[udata$clusters == br,]$AA.JUNCTION,(cel[ela,2]+2),(cel[ela,2]+2)), let[cel[ela,1]]), cl+1 ,cl+2) 
