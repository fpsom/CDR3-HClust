  ...
  function(session, input, output) { 
    ...
    observeEvent(is.null(input$begin),{
      ...
      output$logolev <- renderPlot({
        if (is.null(input$fileIn)) return()
        lole = LogoLev(input$logo_level,lastlist,Clus,df,flagtic,logFile)
        if (is.null(lole)){
      ...
      output$downloadLogoLevel <- downloadHandler(
        ...
        content = function(file) {
          ...
          plot(LogoLev(input$logo_level,lastlist,Clus,df,flagtic,logFile))
          dev.off()
        }) 
      ...
      output$optlev <- renderText({
        ...
        if(input$opt_type == "Identity"){
          fgh = sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[1],Clus$seqnum[t1[1]+1],Opt(AminoCl(t1[1],lastlist,df,flagtic,logFile),flagtic,logFile))
          for(i in 2:length(t1)){
            if(i<= length(xm2)){
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt(AminoCl(t1[i],lastlist,df,flagtic,logFile),flagtic,logFile)),sep = "\n")
            }else{
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d        :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt(AminoCl(t1[i],lastlist,df,flagtic,logFile),flagtic,logFile)),sep = "\n")
            }
          }
        }else if (input$opt_type == "IdentityFreq"){
          fgh = sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[1],Clus$seqnum[t1[1]+1],OptNew(AminoCl(t1[1],lastlist,df,flagtic,logFile),logFile))
          for(i in 2:length(t1)){
            if(i<= length(xm2)){
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[i],Clus$seqnum[t1[i]+1],OptNew(AminoCl(t1[i],lastlist,df,flagtic,logFile),logFile)),sep = "\n")
            }else{
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d        :    %s',t1[i],Clus$seqnum[t1[i]+1],OptNew(AminoCl(t1[i],lastlist,df,flagtic,logFile),logFile)),sep = "\n")
            }
          }
        }else if (input$opt_type == "Similarity"){
          fgh = sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[1],Clus$seqnum[t1[1]+1],Opt2(AminoCl(t1[1],lastlist,df,flagtic,logFile),sim,FALSE,altsim,flagtic,logFile))
          for(i in 2:length(t1)){
            if(i<= length(xm2)){
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt2(AminoCl(t1[i],lastlist,df,flagtic,logFile),sim,FALSE,altsim,flagtic,logFile)),sep = "\n")
            }else{
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d       :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt2(AminoCl(t1[i],lastlist,df,flagtic,logFile),sim,FALSE,altsim,flagtic,logFile)),sep = "\n")
            }
          }
        }else if(input$opt_type == "SimilarityFreq"){
          fgh = sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[1],Clus$seqnum[t1[1]+1],Opt2New(AminoCl(t1[1],lastlist,df,flagtic,logFile),sim,logFile))
          for(i in 2:length(t1)){
            if(i<= length(xm2)){
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d - leaf :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt2New(AminoCl(t1[i],lastlist,df,flagtic,logFile),sim,logFile)),sep = "\n")
            }else{
              fgh = paste(fgh,sprintf('cluster.%d - seqnum:%d        :    %s',t1[i],Clus$seqnum[t1[i]+1],Opt2New(AminoCl(t1[i],lastlist,df,flagtic,logFile),sim,logFile)),sep = "\n")
            }
          }
        }
        fgh
