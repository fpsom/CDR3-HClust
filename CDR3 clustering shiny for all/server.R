...
shinyServer( 
  function(session, input, output) { 
    ...
    observeEvent(is.null(input$begin),{
      ...
      #observeEvent(input$Network == FALSE,{
      #  netlist = Netw(input$net_level,input$thr_value,input$thr_style,input$net_style,df,lastlist,Clus,sim,altsim,input$table_show,FALSE,input$sil)
      #})
      ...
      observeEvent(input$Apply, {
        ...
        output$net <- renderPlot({
          if (is.null(dir()) | is.null(input$Dataset)) return()
          #Netw(input$net_level,input$thr_value,input$thr_style,input$net_style,df,lastlist,Clus,sim,altsim,input$table_show,FALSE,input$sil)
          if(netyp == "whole"){
            par(mfrow = c(1,2))
