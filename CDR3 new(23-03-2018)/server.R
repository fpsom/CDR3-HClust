  ...
  function(session, input, output) { 
    ...
    observeEvent(is.null(input$begin),{
      ...
      output$tre <- renderGrViz({
        if (is.null(input$fileIn)) return()
        Den(input$tree_level,df,lastlist,Clus,flagtic,input$algorithm_type,input$thr_1,input$thr_2,input$thr_en,logFile)
      })
    ...
    output$g1 <- renderUI({
      conditionalPanel(
        ...
        numericInput("thr_1", "Select threshold percentage for number of sequences", 5,  min = 0, max = 100, width="140px"),
        numericInput("thr_2", "Select threshold percentage for Identity or Similarity", 10,  min = 0, max = 100, width="140px"),
        br(),
